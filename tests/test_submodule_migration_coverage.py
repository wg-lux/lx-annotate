...
def test_read_project_version_and_contract_exports(tmp_path):
    ...
    sys.modules["lx_dtypes.models.contracts"] = SimpleNamespace(
        PdfRedactionRequest=object()
    )
    assert urls._has_required_base_api_contracts() is False

    sys.modules["lx_dtypes.models.contracts"] = SimpleNamespace(
        **{name: object() for name in urls.REQUIRED_BASE_API_CONTRACT_EXPORTS}
    )
    assert urls._has_required_base_api_contracts() is True
...
def test_urls_import_mounts_base_api_when_installed_contracts_match(

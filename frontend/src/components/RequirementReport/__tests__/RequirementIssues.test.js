...
describe('RequirementIssues', () => {
    ...
    it('renders backend validation errors from errors[]', async () => {
        ...
        expect(wrapper.text()).toContain('knowledge base validation failed | missing report template');
    });
    ...
    it('renders grouped unmet requirements when the backend response is partial', async () => {
        ...
        await flushPromises();
        expect(wrapper.text()).toContain('Um den Report abzuschließen, müssen die folgenden Voraussetzungen erfüllt sein:');

...
describe('RequirementGenerator visibility', () => {
    beforeEach(() => {
        ...
        vi.mocked(axiosInstance.post).mockResolvedValue({ data: { token: 'lookup-token' } });
    });
    ...
    it('renders the error banner when a visible error exists', async () => {
        ...
        await flushPromises();
        wrapper.vm.error = 'knowledge base validation failed';
        await flushPromises();
    ...
    it('forwards the patient birth date unchanged when creating a case', async () => {
        ...
        vi.mocked(axiosInstance.post)
            ...
            .mockResolvedValueOnce({ data: { token: 'lookup-token-91' } });
        vi.mocked(axiosInstance.get).mockResolvedValue({
            data: {
        ...
        await flushPromises();
        expect(vi.mocked(axiosInstance.post)).toHaveBeenNthCalledWith(1, '/api/patient-examinations/', expect.objectContaining({

...
// Type Guards (same as in component)
...
const patientClassificationsSafe = (arr: any[]): any[] =>
...
const safeMapPatientClassifications = (
    arr: any[], 
    mapFn: (cls: any, index: number) => any,
    findingId?: number
): any[] => {
...
describe('AddableFindingsDetail Crash Guards', () => {
    ...
    describe('Key Generation Safety', () => {
        it('should generate safe keys even with undefined classification data', () => {
            ...
            const keys = unsafeData.map((item, idx) => 
                `pc-123-${idx}-${item?.classification?.id ?? 'none'}`
            );
    ...
    describe('Edge Cases', () => {
        it('should handle null/undefined arrays', () => {
            expect(patientClassificationsSafe(null as any)).toEqual([]);
            expect(patientClassificationsSafe(undefined as any)).toEqual([]);
    ...
    describe('Safe Mapping Function', () => {
        ...
        it('should handle completely invalid input arrays', () => {
            const result1 = safeMapPatientClassifications(null as any, cls => cls);
            const result2 = safeMapPatientClassifications(undefined as any, cls => cls);

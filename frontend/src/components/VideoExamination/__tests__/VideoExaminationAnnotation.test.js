...
describe('Draft System Integration', () => {
    ...
    it('DraftManager.vue - should list drafts and allow deletion', async () => {
        ...
        expect(drafts.find((d) => d.note === 'Test Polyp')).toBeDefined();
        expect(drafts.find((d) => d.note === 'Test Blood')).toBeDefined();
        ...
        expect(updatedDrafts.find((d) => d.note === 'Test Polyp')).toBeUndefined();
    });
    it('should persist drafts to localStorage', () => {
        ...
        store.saveDraft('video-1', annotation);
        // Assert
        expect(window.localStorage.setItem).toHaveBeenCalledWith('lx-annotate-drafts', expect.stringContaining('"video-1"'));
    });

...
describe('useLookupActions payload normalization', () => {
    ...
    it('uses structured backend errors when available', async () => {
        ...
        expect(errorMessage.value).toBe('knowledge base validation failed | missing report template');
    });
    it('marks the session as expired on 404 responses', async () => {
        ...
        expect(errorMessage.value).toBe('Der Fallkontext ist abgelaufen. Bitte im Fall-Setup neu initialisieren.');
    });

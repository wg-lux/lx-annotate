...
export function useFileUrl() {
  ...
  const urlExpiresAt = ref<Date | null>(null)
  ...
  const isUrlExpired = computed(() => {
    if (!urlExpiresAt.value) return false
    return new Date() >= urlExpiresAt.value
  })

  const timeUntilExpiry = computed(() => {
    if (!urlExpiresAt.value) return 0
    const now = new Date()
  ...
  // Methods
  async function loadReportWithSecureUrl(reportId: number): Promise<void> {
    loading.value = true
    error.value = ''
  ...
  async function generateSecureUrl(reportId: number, fileType: string = 'pdf'): Promise<void> {
    loading.value = true
    error.value = ''
  ...
  async function validateCurrentUrl(): Promise<boolean> {
  ...
  async function refreshUrl(reportId: number, fileType: string = 'pdf'): Promise<void> {
    loading.value = true
    error.value = ''
  ...
  async function revokeCurrentUrl(): Promise<void> {
    if (!secureUrl.value) return
  ...
  function setupAutoRefresh(reportId: number, fileType: string = 'pdf', intervalMinutes: number = 30): void {
    clearRefreshTimer()

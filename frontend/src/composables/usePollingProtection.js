...
export function usePollingProtection() {
    const { getStatusSafe, startAnonymizationSafe, validateAnonymizationSafe, clearAllLocks } = useMediaManagement();
    // Soft-locks with TTL (ms)
    const DEFAULT_LOCK_TTL_MS = 5000; // kurzer Zeitraum, damit UI nicht blockiert
    const processingLocks = ref(new Map()); // key -> expiresAt (epoch ms)
    // Helper to create lock key
    const createLockKey = (fileId, mediaType) => {
     ...
     */
    const acquireProcessingLock = (fileId, mediaType, ttlMs = DEFAULT_LOCK_TTL_MS) => {
        purgeExpiredLocks();
        const lockKey = createLockKey(fileId, mediaType);
     ...
     */
    const releaseProcessingLock = (fileId, mediaType) => {
        const lockKey = createLockKey(fileId, mediaType);
        processingLocks.value.delete(lockKey);
    };
    /**
     ...
     */
    const getStatusSafeWithProtection = async (fileId, mediaType) => {
        try {
     ...
     */
    const startAnonymizationSafeWithProtection = async (fileId, mediaType) => {
     ...
     */
    const validateAnonymizationSafeWithProtection = async (fileId, mediaType) => {
        const acquired = acquireProcessingLock(fileId, mediaType);
        if (!acquired) {
     ...
     */
    const clearAllProcessingLocks = async (fileType) => {
        try {

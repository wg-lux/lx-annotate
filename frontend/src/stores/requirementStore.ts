...
export const useRequirementStore = defineStore('requirement', () => {
  ...
  const evaluateRequirements = async (
    requirementSetIds?: number[],
    ...
    try {
      ...
      toast.error({
        text:
          'Fehler bei der Anforderungsevaluierung: ' + (err.response?.data?.detail || err.message),
        timeout: 5000
      })
  ...
  const evaluateRequirementSet = async (
    ...
    try {
      ...
      const requirementSetIds =
        currentRequirementSetIds.value.length > 0
          ? currentRequirementSetIds.value
          : [requirementSetId]
  ...
  const updateRequirementsStatus = (results: RequirementEvaluationResult[]) => {
    ...
    if (currentRequirementSet.value) {
      ...
      currentRequirementSet.value.met = currentRequirementSet.value.requirements.every(
        (req) => req.met
      )
    }
  }

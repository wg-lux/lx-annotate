...
export const useSensitiveMetaStore = defineStore('sensitiveMeta', () => {
  // State
  const currentMetaData = ref<SensitiveMetaData | null>(null)
  const metaDataCache = ref<Map<number, SensitiveMetaData>>(new Map())
  ...
  const isCurrentDataVerified = computed(() => {
    ...
    return !!(
      currentMetaData.value.patient_first_name?.trim() &&
  ...
  async function fetchSensitiveMetaData(options: {
    ...
    try {
      ...
      if (response.data) {
        const metaData: SensitiveMetaData = response.data
  ...
  async function updateSensitiveMetaData(updateData: SensitiveMetaUpdateRequest, mediaType: 'video' | 'pdf' = 'video') {
    ...
    try {
      ...
      if (response.data?.updated_data) {
        const updatedData: SensitiveMetaData = response.data.updated_data
  ...
  async function fetchNextPatient(mediaType: 'video' | 'pdf' = 'video') {
  ...
  function getCachedMetaData(id: number): SensitiveMetaData | null {
  ...
  function validateMetaData(data: Partial<SensitiveMetaUpdateRequest>): {
    isValid: boolean
  ...
  function searchCachedData(searchTerm: string): SensitiveMetaData[] {
    const term = searchTerm.toLowerCase().trim()
    if (!term) return Array.from(metaDataCache.value.values())
    
    return Array.from(metaDataCache.value.values()).filter(data => 
      data.patient_first_name?.toLowerCase().includes(term) ||

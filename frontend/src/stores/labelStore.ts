...
export interface Segment {
  ...
  frameId?: string; // ID des zugehörigen Frames
}
...
export function getSegmentStyle(segment: Segment, duration: number): Record<string, string> {
  if (!duration) return { display: 'none' }; // Vermeidet Division durch Null
...
export const useLabelStore = defineStore('label', () => {
  ...
  const getSegmentsForFrame = (frameId: string) => {
  ...
  async function fetchSegments(videoId?: string) {
    ...
    try {
      ...
      segments.value = response.data.map((seg: any) => ({

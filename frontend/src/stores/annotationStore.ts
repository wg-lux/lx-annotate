...
export const useAnnotationStore = defineStore('annotation', () => {
  ...
  async function loadAnnotations(videoId?: string) {
    ...
    try {
      ...
      state.annotations = data.map((item: any) => ({
        ...item,
  ...
  async function bulkDeleteAnnotations(ids: string[]) {
    state.isLoading = true
    state.error = null
  ...
  function setCurrentAnnotation(annotation: Annotation | null) {
  ...
  function setFilter(filter: Partial<AnnotationFilter>) {
  ...
  function setCurrentVideoId(videoId: string | null) {
  ...
  async function exportAnnotations(format: 'json' | 'csv' = 'json') {
    try {
      const queryParams = new URLSearchParams()
      if (state.filter.videoId) queryParams.append('video_id', state.filter.videoId)
      if (state.filter.type) queryParams.append('type', state.filter.type)
...
export function validateAnnotation(annotation: Partial<Annotation>): string[] {
  ...
  if (!annotation.videoId) errors.push('Video ID is required')
  ...
  if (!annotation.userId) errors.push('User ID is required')

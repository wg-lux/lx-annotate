...
function buildReportingRouter(flow: {
  ...
  const reportingPatientExaminationId = (route: { path: string; params: Record<string, unknown> }) => {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    if (stayingWithinSameReportingDraft) return next()
    if (!flow.hasUnpersistedDraftChanges) return next()
...
describe('reporting flow smoke routes', () => {
  let router: ReturnType<typeof buildReportingRouter>
  let flow: {
    hasUnpersistedDraftChanges: boolean
    savingFinalReport: boolean
    flushDraftAutosave: ReturnType<typeof vi.fn>
  }

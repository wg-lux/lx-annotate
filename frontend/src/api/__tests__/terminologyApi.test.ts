...
describe('terminologyApi folder import', () => {
  ...
  it('rejects an empty directory selection', async () => {
    await expect(createTerminologyBundleArchive([])).rejects.toThrow(
      'Die ausgewählten Verzeichnisse enthalten keine Dateien.'
    )
  })
  ...
  it('packages every package directory in a selected collection separately', async () => {
    ...
    expect(strFromU8(firstEntries['first/config.yaml'])).toContain('name: first')
    expect(strFromU8(firstEntries['first/findings/finding.yaml'])).toContain('name: first_finding')
    expect(strFromU8(firstEntries['first/.dependencies/1-second/config.yaml'])).toContain(
      'name: second'
    )
    expect(strFromU8(secondEntries['second/config.yaml'])).toContain('name: second')
    expect(strFromU8(secondEntries['second/.dependencies/0-first/config.yaml'])).toContain(
      'name: first'
    )
  })
  ...
  it('rejects files outside recognized package directories', async () => {
    ...
    await expect(createTerminologyBundleArchives([config, unrelated])).rejects.toThrow(
      'Dateien außerhalb erkannter Paketverzeichnisse'
    )
  })

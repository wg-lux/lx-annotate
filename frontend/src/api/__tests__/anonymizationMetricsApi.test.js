...
import { buildAnonymizationMetricsQueryParams, fetchAnonymizationMetrics } from '@/api/anonymizationMetricsApi';
...
describe('anonymizationMetricsApi', () => {
    ...
    it('converts frontend filter names to backend snake_case query params', () => {
        expect(buildAnonymizationMetricsQueryParams({
            dateFrom: '2026-04-19',
            dateTo: '2026-05-19',
            mediaType: 'video',
            centerId: 7,
    ...
    it('fetches metrics through the endoreg API endpoint and stores only whitelisted aggregates', async () => {
        ...
        expect(vi.mocked(axiosInstance.get)).toHaveBeenCalledWith('api/media/anonymization/metrics/', {
            params: {
                date_from: '2026-04-19',
                date_to: '2026-05-19',
                media_type: 'pdf'
        ...
        expect(result.fieldQuality[0].rawText).toBeUndefined();
    });
});

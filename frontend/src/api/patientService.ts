...
export const patientService = {
  async getPatients(): Promise<Patient[]> {
    try {
      const response: AxiosResponse<Patient[] | PatientListResponse> = await axiosInstance.get(
        r('patients/')
      )
  ...
  async getGenders(): Promise<Gender[]> {
    try {
      ...
      const response: AxiosResponse<Gender[]> = await axiosInstance
        .get(r('genders/'))
        .catch(async () => {
  ...
  async getCenters(): Promise<Center[]> {
    try {
      ...
      const response: AxiosResponse<Center[]> = await axiosInstance
        .get(r('centers/'))
        .catch(async () => {
          // Fallback über andere verfügbare Endpunkte
          return {
            data: [{ id: 1, name: 'Hauptzentrum', nameDe: 'Hauptzentrum' }] as Center[],
            status: 200,
            statusText: 'OK',
            headers: {},
            config: {} as any

...
export class DateConverter {
     ...
     * @returns true if date1 <= date2, false otherwise
     */
    static isBeforeOrEqual(date1, date2) {
     ...
     * @returns true if date1 >= date2, false otherwise
     */
    static isAfterOrEqual(date1, date2) {
        const result = this.compare(date1, date2);
        return result === 1 || result === 0;
    }
    /**
...
export class DateValidator {
     ...
     */
    addField(fieldName, value, expectedFormat) {
        if (!value) {
     ...
     */
    addConstraint(constraintName, condition, errorMessage) {
        if (!condition) {

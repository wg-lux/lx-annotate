...
describe('DateValidator', () => {
    ...
    it('should validate custom constraints', () => {
        ...
        validator.addConstraint('DOB_BEFORE_EXAM', DateConverter.isBefore(dob, examDate), 'Geburtsdatum muss vor Untersuchungsdatum liegen');
        expect(validator.hasErrors()).toBe(true);
    ...
    it('should validate complete patient data scenario', () => {
        ...
        if (dobISO && examISO) {
            validator.addConstraint('DOB_BEFORE_EXAM', DateConverter.isBefore(dobISO, examISO), 'Geburtsdatum muss vor Untersuchungsdatum liegen');
        }
        expect(validator.hasErrors()).toBe(false);
        expect(validator.getSummary()).toBe('Alle Datumsfelder sind gültig');
    });

...
function buildFlowStore() {
    const flow = reactive({
        ...
        setTemplateSelection: vi.fn((params) => {
            ...
            if (params.templateName !== undefined)
                flow.selectedTemplateName = params.templateName || null;
        }),
        ...
        removeFinding: vi.fn((findingLocalId) => {
            flow.currentRuntimeDraft.payload.patientFindings =
                flow.currentRuntimeDraft.payload.patientFindings.filter((finding) => finding.localId !== findingLocalId);
        }),
        updateClassificationValue: vi.fn((params) => {
            const finding = flow.currentRuntimeDraft.payload.patientFindings.find((entry) => entry.localId === params.findingLocalId);
            if (!finding)
                return;
            finding.classificationChoices = finding.classificationChoices.filter((entry) => entry.classification !== params.classificationName);
        ...
        noteFindingAdded: vi.fn((findingId) => {
        ...
        noteClassificationUpdated: vi.fn((findingId, classificationId, choiceId) => {
            flow.findingsRevision += 1;
            flow.lastFindingsEvent = {
...
describe('FindingsCapturePage runtime draft flow', () => {
    beforeEach(() => {
        ...
        hoisted.findingSelectorsRef.current = {
            ...
            getFindingById: vi.fn().mockImplementation((id) => id === 11
                ? {
                    id: 11,
                    name: 'esophagus_polyp',
                    displayName: 'Oesophagus Polyp'

...
describe('Response-Interceptor', () => {
    test('wandelt snake_case zu camelCase', async () => {
      ...
      const result = await (axiosInstance.interceptors.response as any).handlers[0].fulfilled(response)
...
describe('CSRF-Interceptor', () => {
  test('setzt X-CSRFToken, wenn Cookie existiert', async () => {
    ...
    // Cast zu any, um auf die interne handlers-Eigenschaft zuzugreifen
    const config = await (axiosInstance.interceptors.request as any).handlers[0].fulfilled({ headers: {} })
  ...
  test('lässt Header unverändert, wenn kein Cookie', async () => {
    ...
    // Cast zu any, um auf die interne handlers-Eigenschaft zuzugreifen
    const config = await (axiosInstance.interceptors.request as any).handlers[0].fulfilled({ headers: { foo: 'bar' } })

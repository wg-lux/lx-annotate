...
describe('RequirementGenerator', () => {
  ...
  describe('Create PatientExamination + init lookup (happy path)', () => {
    it('should successfully create patient examination and initialize lookup', async () => {
      ...
      ;(axiosInstance.get as MockedFunction<any>).mockResolvedValueOnce({ data: mockLookupData })
      ...
      expect(mockPatientExaminationStore.addPatientExamination).toHaveBeenCalledWith(
        mockPatientExaminationResponse
      )
  ...
  describe('AddableFindingsDetail finding-added event', () => {
    it('should show success message and trigger evaluation when finding is added', async () => {
      ...
      expect(wrapper.text()).toContain(
        'Befund "right_flexure" wurde erfolgreich hinzugefügt mit 1 Klassifikation!'
      )
  ...
  describe('FindingsDetail classification-updated event', () => {
    it('should show success message and trigger evaluation when classification is updated', async () => {
      ...
      const updateButton = wrapper.find(
        '[data-testid="findings-detail-1"] [data-testid="update-classification-btn"]'
      )
      await updateButton.trigger('click')
      await nextTick()

      expect(wrapper.text()).toContain(
        'Klassifikation für "Test Finding" wurde erfolgreich ausgewählt!'
      )
  ...
  describe('Token validation on mount', () => {
    it('should validate existing token and fetch data on mount', async () => {
      ...
      expect(axiosInstance.get).toHaveBeenCalledWith(
        '/api/lookup/existing-token/parts/?keys=patientExaminationId'
      )
      expect(axiosInstance.get).toHaveBeenCalledWith(
        '/api/lookup/existing-token/all/?skip_recompute=true'
      )
    })
  ...
  describe('Manual session management', () => {
    it('should renew session manually', async () => {
      ;(axiosInstance.get as MockedFunction<any>).mockResolvedValue({
        data: { patientExaminationId: 80 }
      })
      ;(axiosInstance.patch as MockedFunction<any>).mockResolvedValue({ data: {} })
      ...
      expect(axiosInstance.get).toHaveBeenCalledWith(
        '/api/lookup/test-token/parts/?keys=patientExaminationId'
      )
      expect(axiosInstance.patch).toHaveBeenCalledWith('/api/lookup/test-token/parts/', {
        updates: {}
      })
  ...
  describe('Finding status logic', () => {
    it('should correctly identify added findings', async () => {
      ...
      const findingDetail1 = wrapper.findComponent({
        name: 'FindingsDetail',
        props: { 'finding-id': 1 }
      })
      const findingDetail2 = wrapper.findComponent({
        name: 'FindingsDetail',
        props: { 'finding-id': 2 }
      })

...
def main():
    ...
    if args.cmd == "collect":
        ...
        fail = (
            pathlib.Path(args.faillog).read_text(encoding="utf-8")
            if args.faillog and pathlib.Path(args.faillog).exists()
            else ""
        )
        snip = (
            pathlib.Path(args.snippets).read_text(encoding="utf-8")
            if args.snippets and pathlib.Path(args.snippets).exists()
            else ""
        )
        write_text(
            args.out,
            json.dumps(
                {"vitest": vj, "coverage": cov, "failLog": fail, "setupSnippets": snip},
                indent=2,
            ),
        )

...
export function buildArtifacts({
  ...
  const failLog =
    failLogPath && fs.existsSync(failLogPath) ? fs.readFileSync(failLogPath, 'utf8') : ''
  const setupSnippets =
    setupSnippetsPath && fs.existsSync(setupSnippetsPath)
      ? fs.readFileSync(setupSnippetsPath, 'utf8')
      : ''

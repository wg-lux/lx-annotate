...
APP_DIR="${APP_DIR:-/var/lib/lx-annotate}"
VENV_DIR="${VENV_DIR:-$APP_DIR/.venv}"
ENV_FILE="${ENV_FILE:-$APP_DIR/.env.systemd}"
...
export DJANGO_STATIC_ROOT="${DJANGO_STATIC_ROOT:-$APP_DIR/staticfiles}"
export LX_ANNOTATE_DATA_DIR="${LX_ANNOTATE_DATA_DIR:-$APP_DIR/data}"
export XDG_DATA_HOME="${XDG_DATA_HOME:-$APP_DIR}"
export TESSDATA_PREFIX="${TESSDATA_PREFIX:-/usr/share/tesseract-ocr/5/tessdata}"
export PYTORCH_ALLOC_CONF="${PYTORCH_ALLOC_CONF:-expandable_segments:True}"

mkdir -p "$DJANGO_STATIC_ROOT" "$LX_ANNOTATE_DATA_DIR"

        ...
        for path in ld_path.split(":"):
            if os.path.exists(path):
                try:
                    ...
                    if cuda_files:
                        cuda_libs_found.extend(
                            [os.path.join(path, f) for f in cuda_files[:3]],
                        )  # Show first 3
                except OSError:
                    pass
...
try:
    ...
    try:
        ...
        result = cuda_init(0)
        print(
            f"   cuInit(0) result: {result} ({'SUCCESS' if result == 0 else 'FAILED'})",
        )
        ...
        if result == 0:
            ...
            print(
                f"   cuDeviceGetCount result: {count_result}, count: {device_count.value}",
            )

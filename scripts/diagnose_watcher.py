...
def check_watchdog_logging():
    ...
    for logger_name in watchdog_loggers:
        ...
        if level <= logging.DEBUG:
            print("     ⚠️  DEBUG level will log every event!")
        elif level <= logging.INFO:
            print("     ℹ️  INFO level may be chatty")
        else:
            ...
            for subdir in subdirs[:5]:  # Limit to first 5
                try:
                    ...
                    if "tmp" in subdir.name or "transcoding" in subdir.name:
                        print("       ⚠️  Temp directory - high event volume expected!")
...
def check_log_files():
    ...
    if log_file.exists():
        ...
        try:
            ...
            print("     Last 100 lines analyzed")
            print(f"     DEBUG lines: {debug_lines}")

            if debug_lines > 50:
                print("       ⚠️  High DEBUG volume - check logging levels!")

            if event_counts:
                print(f"     Event counts: {event_counts}")
                total_events = sum(event_counts.values())
                if total_events > 20:
                    print("       ⚠️  High event volume detected!")

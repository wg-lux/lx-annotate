...
- `requirement_set.module_name` -> selected module (default `report_template_examples`)
...
- `requirements[i].kind`:
  - `findings_validator` if from `template.validators.findings_validators`
...
- `result.met` -> validator execution `ok`
- `result.status` -> `PASSED` / `FAILED`
- `result.details` -> compact message from validator issues
